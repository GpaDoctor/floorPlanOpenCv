...
solid_green, green_mask = solidGreen(green_only)
...
white_before_wall_mask = cv2.countNonZero(white_mask_before)
...
wall_mask_pixels = int(np.count_nonzero(wall_mask))
...
white_after_wall_mask = cv2.countNonZero(white_mask_after)

white_removed_by_wall_mask = max(0, white_before_wall_mask - white_after_wall_mask)

if white_before_wall_mask > 0:
    white_removed_ratio = white_removed_by_wall_mask / white_before_wall_mask
    white_retained_ratio = white_after_wall_mask / white_before_wall_mask
else:
...
success = cv2.imwrite(str(output_file), combined)

if not success:
    raise RuntimeError(f"Unable to save processed image: {output_file}")
...
# ---------------------------------------------------------
debug_before_file = output_file.with_name(f"{output_file.stem}_white_before.png")

debug_after_file = output_file.with_name(f"{output_file.stem}_white_after.png")

debug_wall_mask_file = output_file.with_name(f"{output_file.stem}_wall_mask.png")

# cv2.imwrite(str(debug_before_file), white_mask_before)

# cv2.imwrite(str(debug_after_file), white_mask_after)

# cv2.imwrite(str(debug_wall_mask_file), np.where(wall_mask, 255, 0).astype(np.uint8))
...
processing_stats = {
    ...
    "white_pixels_before_wall_mask": int(white_before_wall_mask),
    "white_pixels_after_wall_mask": int(white_after_wall_mask),
    "white_pixels_removed_by_wall_mask": int(white_removed_by_wall_mask),
    "wall_mask_pixels": wall_mask_pixels,
    "white_removed_ratio": float(white_removed_ratio),
    "white_retained_ratio": float(white_retained_ratio),
}

with open(stats_file, "w", encoding="utf-8") as stats_handle:
    json.dump(processing_stats, stats_handle, indent=4)


print("")
print("=" * 60)
print("[WALL MASK AREA DIAGNOSTICS]")
print(f"Image dimensions: " f"{combined.shape[1]} x {combined.shape[0]} px")
print(f"White pixels before wall mask: " f"{white_before_wall_mask:,}")
print(f"White pixels after wall mask: " f"{white_after_wall_mask:,}")
print(f"White pixels removed: " f"{white_removed_by_wall_mask:,}")
